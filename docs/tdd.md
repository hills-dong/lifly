...
> Version: 0.2 | Date: 2026-03-10 | Status: Approved (M1 complete)
>
...
| 模型 | 用途 | 模式 |
|------|------|------|
| **Gemini 3.1 Pro** | 文本理解（todo 解析）、视觉理解（证件 OCR） | text / vision |
| **Gemini 3.1 Flash Image** | 图像生成/处理（证件图片标准化） | image_generation |
...
AtomicCapability 区分运行时类型。M1 支持 `builtin`、`remote_llm`、`script`（script 未实现）三种。`remote_llm` 通过 Google AI Studio API 调用 Gemini 模型，在 `runtime_config` 中指定 model、mode、system_prompt、temperature。M1 使用 Gemini 3.1 Pro（文本/视觉）和 Gemini 3.1 Flash Image（图像生成）。未来端侧模型成熟后，可将 remote 逐步切换为 local，对上层透明。
...
创建 Pipeline (tool: Todo, status: running)
    ...
    │
    ├─→ StepExecution #2: todo_parse [Gemini 3.1 Pro, text]
    │       LLM 识别结构化信息：
    │       {
    │           "title": "开会讨论项目方案",
...
创建 Pipeline (tool: 证件管理, status: running)
    ...
    │
    ├─→ StepExecution #2: ocr_extract [Gemini 3.1 Pro, vision]
    ...
    │       }
    │
    ├─→ StepExecution #3: image_process [Gemini 3.1 Flash Image, image_generation]
...
3. Todo 端到端联调：文本输入 → Gemini 3.1 Pro 解析 → DataObject → Reminder
4. 证件管理工具 seed：注册 Tool + ToolVersion + 5 步 ToolStep + data_schema
5. 证件端到端联调：图片上传 → Gemini Pro OCR → Gemini Flash Image 标准化 → DataObject → Reminder
